...
usage: $name -t <type> [--stdout|--noheaders] file.txt
...
#my_version = '0.5'
#
#usage = 'usage: %s [--noheaders] --type <type> [file.txt | -]\n'%(sys.argv[0])
#
## yeah, it's dirty. someday i'll use getopts.
#if len(sys.argv) == 4:
#	doctype = sys.argv[2]; file = sys.argv[3]; noheaders = 0
#elif len(sys.argv) == 5 and sys.argv[1] == '--noheaders':
#	doctype = sys.argv[3]; file = sys.argv[4]; noheaders = 1
#else:
...
#			if not noheaders: printHeader(title,author,date)
...
#	if noheaders and linenr == 1 and doctype != 'pm6': print TAGparagraph[T]
#
## comments
#	# just skip them
...
#					line = line+'<%s>%s</%s>\n'%(celltag,string.strip(cel),celltag)
#			line = '<tr>\n%s</tr>'%line
#
#
#
