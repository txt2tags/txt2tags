...
\usepackage[pdfstartview=FitV,urlcolor=blue,colorlinks=true,bookmarks=true]{hyperref}
...
class CommandLine:
	...
	def _compose_long_opts(self):
		...
		ret.extend(['no-style','no-encoding','no-outfile'])  # turn OFF
	...
	def get_raw_config(self, cmdline=[], ignore=[], filter=[], relative=0):
		...
		for name,value in opts:
			...
			if name == 'outfile' and value != STDOUT and relative:
...
class SourceDocument:
	...
	def scan(self, lines):
		...
		buf = lines
		if len(buf) == 0:
			Error(_('The input file is empty: %s')%filename)
...
class ConfigMaster:
	...
	def get_outfile_name(self, config={}):
		...
		if outfile and outfile != STDOUT and not os.path.isabs(outfile):
...
def doHeader(headers, config):
	...
	if target in ['html','xhtml'] and config.get('css-inside') and \
	   config.get('style'):
		...
		template = string.replace(template, '</HEAD>', css+'</HEAD>')
...
def process_source_file(file='', noconf=0, contents=[]):
	...
	if not noconf:
		...
		if contents:
			full_parsed['sourcefile'] = '-module-'
			full_parsed['infile'] = '-module-'
		else:
...
def convert_this_files(configs, dump_results=0):
	global CONF
