...
#my_version = '0.6'
#
#f_noheaders = f_enumtitle = 0
#doctype = ''
#usage = 'usage: %s [--noheaders|--enumtitle] --type <type> file.txt|-\n'%(sys.argv[0])
#
## get cmdline options
#errormsg = 'bad option or missing argument. try --help.'
#try: (opt, args) = getopt.getopt(sys.argv[1:], '',
#     ['noheaders', 'enumtitle', 'type='])
...
#TAGnumlistItem = ['\a. ', '<item>', '<LI>', '•	', '\a. ', '\a. ', '\a. ']
...
#TAGtableOpen     = [ '', '<table><tabular ca="c">', '<table align=center border=1>', '', '', '', '']
#TAGtableLineOpen = [ '', '', '<tr>', '', '', '||', '']
...
#re_underline = re.compile(r'__([^_].*?)__') # underline lead/trailing blank makes sense
...
#  'punct' : r'.,;:!?'
#}
#retxt_url = r'\b(%s([%s]+(:%s)?@)?|%s)[%s]+(#[%s]+|\?[%s]+)?(?=[%s]|[^%s]|$)\b'%(
#             urlskel['proto'],urlskel['login'],urlskel['pass'],
#             urlskel['guess'],urlskel['chars'],urlskel['anchor'],
...
#				while len(listident):
#					tag = TAGlistClose[T]
#					if listids[-1] == '+': tag = TAGnumlistClose[T]
...
#			tag = TAGlistOpen[T]
#			if listids[-1] == '+': tag = TAGnumlistOpen[T]
...
#				tag = re_x.sub(`listcount[-1]`, tag)
#		if doctype == 'mgp': listid = len(listident)*'\t'
#		line = re_list.sub(listid+tag+r'\3',line)
#		if listholdspace:
...
#				print TAGtableOpen[T] ; istableaware = 1
#			else:
#				print TAGareaPreOpen[T] ; istableaware = 0
#		
#		istable = 1
#		
#		if istableaware:
...
#					else: cel = string.replace(cel,'\|', '|') # user escaped (not delim!)
#					if not tablecel and doctype == 'sgml': tc2 = '' # last cell
